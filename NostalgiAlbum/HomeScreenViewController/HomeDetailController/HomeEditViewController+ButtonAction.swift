...
extension HomeEditViewController {
    ...
    @IBAction func saveAlbum(_ sender: Any) {
        ...
        // 앨범을 수정하는 경우
            ...
            // 앨범 폴더 명 변경
            do{
                // 폴더 명이 변경된 경우에만 이름을 변경 -> 같은 경우 변경하면 오류 발생
                if albumDirectoryPath != modifyAlbumDirectoryPath {
                    try FileManager.default.moveItem(at: albumDirectoryPath, to: modifyAlbumDirectoryPath)
                }
            } catch let error {
                print("albumDirectoryPath: \(albumDirectoryPath) \n modifyAlbumDirectoryPath: \(modifyAlbumDirectoryPath)")
            ...
            if albumName.text! != albumNameBeforeModify {
